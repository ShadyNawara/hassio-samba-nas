...
function mount_disk() { # $1 disk $2 path $3 remote_mount
     ...
     if findmnt -n -o TARGET $path/$disk >/dev/null 2>&1; then
          ...
          ssh root@${ipaddress%/*} -p 22222 -o "StrictHostKeyChecking no" "if grep -qs '/media/$disk ' /proc/mounts; then echo 'Disk $disk already mounted on host' ; else  mount -t auto $devpath/$disk /media/$disk -o nosuid,relatime,noexec; fi" \
